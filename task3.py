...
def extract_hog_features(image, bbox):
    ...
    # roi_resized = cv2.resize(roi, (64, 128))   # Resize using custom function
...
def load_data(coco_file, image_base_dir):
    ...
    if person_category_id is None:
        raise ValueError("person category ID not found in categories")
    ...
    if not image_paths:
        raise ValueError("No image paths found for person annotations")

    return image_paths, person_category_id

def prepare_dataset(image_paths, person_category_id):
    X = []
    y = []
...
def task3():
    ...
    train_image_paths, train_person_category_id = load_data(train_coco_file, train_image_base_dir)
    X_train, y_train = prepare_dataset(train_image_paths, train_person_category_id)

    # Load and prepare the testing dataset
    test_image_paths, test_person_category_id = load_data(test_coco_file, test_image_base_dir)
    X_test, y_test = prepare_dataset(test_image_paths, test_person_category_id)
